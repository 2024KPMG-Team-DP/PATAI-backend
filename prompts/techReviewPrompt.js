// OCR 결과 분류 prompt
const ocrPrompt = `
너에게 ocr로 인식한 텍스트를 줄건데, orc로 인식해서 문장 단어들이 흩어져 있어.
이 텍스트를 기술명(name), 기업명(organization), 작성일자(date), 기술 설명(description), 기존 기술의 문제점(problem), 기술의 개선점(solve), 기술의 기대효과(benefit), 기술의 구성 설명(composition)으로 정확하게 나눠줘.
기술 설명과 기술 구성 설명은 다른 파트이고 네 맘대로 텍스트를 요약하지 말고 원문 그대로를 반환해야해.
이제 답변의 형식에 대해 설명해줄게. 답변은 반드시 아래와 같은 형식으로 생성해줘.
{
  "name": "",
  "organization": "",
  "date": "",
...
유사도가 중하인 경우는 아이디어와 해당 특허가 약간 유사하지만, 유사도가 낮은 경우야. 일부 개념, 기술적인 부분, 용도, 구현적인 측면에서 유사하지 않은 부분이 확인되는 수준이야. 이 경우는 특허 침해 위험이 낮아.
유사도가 하인 경우는 아이디어와 해당 특허 간의 관련성이 거의 없을 때야.
이제 답변 형식에 들어갈 필드들을 설명해줄게.
similars는 유사한 특허들의 정보와 분석 결과가 객체 형태로 포함되는 배열이야.
similars에 포함되는 유사 특허 객체의 필드는 name, registration, analysis, similarity야.
similars 배열에는 유사도가 중하 또는 하인 특허를 제외한 모든 관련 특허들이 객체로 만들어져 들어가야 해.
즉 너가 판단하기에 유사도가 중 이상이면 배열 안에 포함되어야 한다는 거지. 0개부터 3개까지 유동적으로 구성하도록 해.
name은 특허의 이름, registration은 특허의 등록번호, analysis는 너가 유사도를 분석한 결과, similarity는 너가 판단한 유사도에 해당해.
conclusion에는 아이디어 출원에 대한 전체적인 분석 결과,
guide에는 분석한 유사도를 최소화하기 위한 개선 방향 가이드 내용이 포함되어야 해.
유사도가 중하 혹은 하인 경우에는 사실상 유사성이 없는 거니까 나열하지 말아줘.
이 문장 다음부터 Sources 전까지는 너가 생성해야 할 답변의 형식이고,
답변은 반드시 아래와 같은 형식으로 생성해줘.
{
  "similars": [
    {
      "name": "",
